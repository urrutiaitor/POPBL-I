	...
	do{
		...
		for (i = 0; i < OSTOPOKOPURUA; i++){
			...
			if (ostopoak[i].kolorea == HARRI_KOLOREA){
				...
				jarraitu = 1;
				for (t = 0; t < 50; t++){
					if ((objetuak[ostopoak[i].x0 + t][ostopoak[i].y1 ] == BERDE_ILUNA) || (objetuak[ostopoak[i].x0 + t][ostopoak[i].y1 ] == HARRI_KOLOREA) || (objetuak[ostopoak[i].x0 + t][ostopoak[i].y1] == HARRI_BOROBILA) || (objetuak[ostopoak[i].x0 + t][ostopoak[i].y1 ] == MOREA_BOTOIA1) || (objetuak[ostopoak[i].x0 + t][ostopoak[i].y1 ] == MOREA_PLATAFORMA1) || (objetuak[ostopoak[i].x0 + t][ostopoak[i].y1 ] == MOREA_BOTOIA2) || (objetuak[ostopoak[i].x0 + t][ostopoak[i].y1 ] == MOREA_PLATAFORMA2)){
						jarraitu = 0;
					}
				}
				if (jarraitu==1){
			...
			ondo = 0;
			if (ostopoak[i].kolorea == DIAMANTE_GORRIA){
				if ((posizioGorria.y >= ostopoak[i].y0 - 14) && (posizioGorria.y <= ostopoak[i].y0 + 14) && (posizioGorria.x + ERRADIOBOLA >= ostopoak[i].x0 - 7) && (posizioGorria.x - ERRADIOBOLA <= ostopoak[i].x0 + 7)){
					ostopoak[i].kolorea = BELTZA;
					jokalariak[0].puntuazioa = jokalariak[0].puntuazioa + 30;
				}
			}

			ondo = 0;
			if (ostopoak[i].kolorea == DIAMANTE_URDINA){
				if ((posizioUrdina.y >= ostopoak[i].y0 - 14) && (posizioUrdina.y <= ostopoak[i].y0 + 14) && (posizioUrdina.x + ERRADIOBOLA >= ostopoak[i].x0 - 7) && (posizioUrdina.x - ERRADIOBOLA <= ostopoak[i].x0 + 7)){
					ostopoak[i].kolorea = BELTZA;
					jokalariak[0].puntuazioa = jokalariak[0].puntuazioa + 30;
				}
			}
		...
		//HELMUGAK KONPROBATU
		if ((objetuak[posizioGorria.x][posizioGorria.y + ERRADIOBOLA + 1] == URDINA) || (objetuak[posizioGorria.x][posizioGorria.y + ERRADIOBOLA + 1] == BERDEA) || (objetuak[posizioUrdina.x][posizioUrdina.y + ERRADIOBOLA + 1] == GORRIA) || (objetuak[posizioUrdina.x][posizioUrdina.y + ERRADIOBOLA + 1] == BERDEA)){
			jokalariak[0].pantailaEgoerak[pantaila] = AMAITUGABE;
			jokalariak[0].puntuazioa = jokalariak[0].puntuazioa + 50;
		}
		...
		if ((objetuak[posizioGorria.x][posizioGorria.y] == GORRIA2) && (objetuak[posizioUrdina.x][posizioUrdina.y] == URDINA2)){
			...
			jokalariak[0].puntuazioa = jokalariak[0].puntuazioa - 20;
		}
		if (emaitza == 1){
			printf("");
		}
		
		if ((controls.keyboard.intro == TRUE)&&(emaitza!=0)){
