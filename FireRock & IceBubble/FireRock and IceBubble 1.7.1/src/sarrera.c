...
void SARRERA_jokoarenAukerak(int x, int y, int zabalera, int altuera){
	...
	escribirTexto(150, 200, RANKING);
	escribirTexto(150, 250, KREDITUAK);
...
void SARRERA_puntuazioa(){
	...
	int tekla = 0;
	char puntuak[10];
	sprintf(puntuak, "%d", jokalariak[0].puntuazioa);
	//programa
	do{
		tekla = ebentoaEntzun();
		borraCopiaPantalla();
		escribirTexto(225, 50, TITULOA);
		escribirTexto(150, 200, "1. Jokalaria:");
		escribirTexto(300, 200, puntuak);
		actualizaPantalla();
	} while (tekla != TECLA_RETURN);
}
